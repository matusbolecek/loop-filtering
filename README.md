...
# Installation
It is required to have python 3.10+ installed on the system. A virtual environment is recommended for this, as many packages will be pulled. Then, run pip install -r requirements.txt to download all the required packages - namely PyTorch, librosa, transformers. Afterwards, you are ready to use the filtering tool. Note that the model will automatically be downloaded on the first run of the program.
