...
class FileProcessing:
    def _process_folder(self, loops_dir) -> list[Path]:
        ...
        if not loops:
            raise ValueError(f"No supported audio files found in: {p} (supported: {formats})")
    ...
    def save(self, name) -> None:
        if self.embeddings:
            if len(self.embeddings) < 5:
                print('Warning! There is <5 embeddings generated. This is generally not recommended for best accuracy')
...
class Compare(FileProcessing):
    def __init__(self, profile_paths: Sequence, threshold: float=0.85):
        ...
        self.base_vectors = [
                np.load(arr) for arr in profile_paths
                ]
